  ...
  stages {
    ...
    stage('Build') {
      steps {
        ...
        dir('tmp') {
          timestamps {
            sh '../scripts/firedrake-install --disable-ssh --minimal-petsc --slepc --documentation-dependencies --install thetis --install gusto --install icepack --no-package-manager --package-branch PyOP2 pefarrell/mat-type-dense || (cat firedrake-install.log && /bin/false)'
          }
        }
      }
    }
